...
resource "aws_launch_configuration" "consul" {
  ...
  image_id                    = var.image_id != "" ? var.image_id : element(data.aws_ami.consul.*.id, 0)
...
resource "aws_autoscaling_group" "consul" {
  ...
  max_size             = var.nodes != -1 ? var.nodes : length(var.subnet_ids)
  min_size             = var.nodes != -1 ? var.nodes : length(var.subnet_ids)
  desired_capacity     = var.nodes != -1 ? var.nodes : length(var.subnet_ids)
  default_cooldown     = 30
  force_delete         = true
